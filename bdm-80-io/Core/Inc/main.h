/* USER CODE BEGIN Header */
/**
  ******************************************************************************
  * @file           : main.h
  * @brief          : Header for main.c file.
  *                   This file contains the common defines of the application.
  ******************************************************************************
  * @attention
  *
  * Copyright (c) 2022 STMicroelectronics.
  * All rights reserved.
  *
  * This software is licensed under terms that can be found in the LICENSE file
  * in the root directory of this software component.
  * If no LICENSE file comes with this software, it is provided AS-IS.
  *
  ******************************************************************************
  */
/* USER CODE END Header */

/* Define to prevent recursive inclusion -------------------------------------*/
#ifndef __MAIN_H
#define __MAIN_H

#ifdef __cplusplus
extern "C" {
#endif

/* Includes ------------------------------------------------------------------*/
#include "stm32f4xx_hal.h"

/* Private includes ----------------------------------------------------------*/
/* USER CODE BEGIN Includes */

/* USER CODE END Includes */

/* Exported types ------------------------------------------------------------*/
/* USER CODE BEGIN ET */

/* USER CODE END ET */

/* Exported constants --------------------------------------------------------*/
/* USER CODE BEGIN EC */

/* USER CODE END EC */

/* Exported macro ------------------------------------------------------------*/
/* USER CODE BEGIN EM */

/* USER CODE END EM */

/* Exported functions prototypes ---------------------------------------------*/
void Error_Handler(void);

/* USER CODE BEGIN EFP */

/* USER CODE END EFP */

/* Private defines -----------------------------------------------------------*/
#define P_MISO_Pin GPIO_PIN_2
#define P_MISO_GPIO_Port GPIOC
#define P_MOSI_Pin GPIO_PIN_3
#define P_MOSI_GPIO_Port GPIOC
#define M_TX_Pin GPIO_PIN_2
#define M_TX_GPIO_Port GPIOA
#define M_RX_Pin GPIO_PIN_3
#define M_RX_GPIO_Port GPIOA
#define T_SCK_Pin GPIO_PIN_5
#define T_SCK_GPIO_Port GPIOA
#define T_MISO_Pin GPIO_PIN_6
#define T_MISO_GPIO_Port GPIOA
#define T_MOSI_Pin GPIO_PIN_7
#define T_MOSI_GPIO_Port GPIOA
#define T_CS_Pin GPIO_PIN_0
#define T_CS_GPIO_Port GPIOB
#define P_SCK_Pin GPIO_PIN_13
#define P_SCK_GPIO_Port GPIOB
#define P_CS_Pin GPIO_PIN_14
#define P_CS_GPIO_Port GPIOB
#define P_RESET_Pin GPIO_PIN_6
#define P_RESET_GPIO_Port GPIOC
#define P_DC_Pin GPIO_PIN_7
#define P_DC_GPIO_Port GPIOC
#define SEG2_DIO_Pin GPIO_PIN_9
#define SEG2_DIO_GPIO_Port GPIOC
#define SEG2_CLK_Pin GPIO_PIN_8
#define SEG2_CLK_GPIO_Port GPIOA
#define SEG2_STB_Pin GPIO_PIN_9
#define SEG2_STB_GPIO_Port GPIOA
#define SEG1_STB_Pin GPIO_PIN_5
#define SEG1_STB_GPIO_Port GPIOB
#define SEG1_CLK_Pin GPIO_PIN_6
#define SEG1_CLK_GPIO_Port GPIOB
#define SEG1_DIO_Pin GPIO_PIN_7
#define SEG1_DIO_GPIO_Port GPIOB

/* USER CODE BEGIN Private defines */

/* USER CODE END Private defines */

#ifdef __cplusplus
}
#endif

#endif /* __MAIN_H */
